...
imgs[1].src = 'P1.png';
imgs[2].src = 'P2.png';
imgs[3].src = 'p1_super.png';
imgs[4].src = 'p2_super.png';
...
function initBoard(bf){
    ...
    Object.keys(SQR).forEach(function(key) {
        if(hasPieceAt(bf.rPieces, SQR[key])){
            bf.board[SQR[key]] = PIECE_TYPE.P2_PIECE;

        }
        if(hasPieceAt(bf.yPieces, SQR[key])){
            bf.board[SQR[key]] = PIECE_TYPE.P1_PIECE;
        }
    });
...
function insertPiece(src, target, bf){
    ...
    if(PIECE_TYPE.P2_PIECE == bf.board[target]){
        bf.rPieces[bf.rPieces.indexOf(src)] = target;
    }else if(PIECE_TYPE.P1_PIECE == bf.board[target]){
